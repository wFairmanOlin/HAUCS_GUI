...
def generate_image():
    ...
    while True:
        if not (uart_connection and uart_connection.connected):
            ...
            if (len(msg)==1) and (msg[0].startswith("df")): #dfinish             
                print("sample done")
                ...
                try:                
                    ...
                    init_DO =sdata['init_do']
                    init_pressure = sdata['init_p']
                    pond_id = sdata['pond_id']
                ...
                ble_uart_write(msg)
                sleep(5)                       
            elif (len(msg)==2) and (msg[0].startswith("b'd")): #b'dsize
                cstr = msg[1]
                slen = len(cstr)
                print(f'strlen: {slen}')
                ...
                if prev_sample_size<=0 and current_sample_size>0:
                    ...
                    if current_sample_size > prev_sample_size: # sampling continuing
                        ...
                        t = np.zeros(size)
                        do = np.zeros(size)                                            
            elif (len(msg)==2): # return get msg              
                keystr=msg[0]
                try:
                    val = float(msg[1]) # assuming only query the 
                    sdata[keystr] = val
                    save_json() 
                    print(f'{keystr}:{val}')
                except: #if a string
                    val = msg[1] # assuming only query the 
                    sdata[keystr] = val
                    save_json() 
                    print(f'{keystr}:{val}')
                sleep(1)

            elif (len(msg)==3) and msg[0].startswith('ds'):#dstart
                print("starting sample")
                csv_file = init_file()
                cnt = 0               
                size = int(msg[1])
...
def main():
    ...
    #os.popen('sudo systemctl restart bluetooth')
    sleep(5)
    try:
        ble_connect()
    except:
